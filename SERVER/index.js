...
//deleteaBM
app.delete('/api/deleteabm', verifyJWT, (req,res)=>{
    const role = req.user.role;
    if(role==="admin"){
    const {id} = req.body;
    const filter = {_id:id}
...
app.put('/api/:id/updatebm', verifyJWT, (req,res) => {
    //destructuring data from request and body
    const role = req.user.role;
    if(role==="admin"){
    const id = req.params.id;
    const filter = {_id: id};
    const {email, name, designation, gender, password, role, batch, course, phone} = req.body;
    const update = {email, name, designation, gender, password, role, batch, course, phone}
    Users.updateOne (filter, update,{new: true})
    .then((batch)=>{
        res.json(batch)
    })
    }else{
